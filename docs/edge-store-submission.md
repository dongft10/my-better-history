...
> 目标：将 MyBetterHistory 提交到 Microsoft Edge 扩展商店（Edge Add-ons）。
...
- [ ] 打包为 **ZIP**，`manifest.json` 位于 zip 根目录
  ...
  \- ✅ 已验证：zip 含 manifest、background.js、index.html/assets、icons 16/32/48/128、\_locales 双语
- [ ] manifest\_version 3
- [ ] `name` / `version`（1.2.2）/ `description` 已就绪（描述已改为中性文案，不提及特定浏览器/商店）
- [ ] 图标：16 / 32 / 48 / 128 PNG 齐全（已补充 32）
- [ ] 不包含 `update_url`（商店自动管理更新）
- [ ] `default_locale: "en"` 且包内含 `_locales/`（构建脚本已处理）
- [ ] 权限与 host\_permissions 已在 PRIVACY.md 中说明

## 三、商店列表（Store Listing）素材

| 素材                     | 要求                                    | 状态                                 |
| ---------------------- | ------------------------------------- | ---------------------------------- |
| 名称 Name                | ≤ 45 字符（"MyBetterHistory" = 15 ✓）   | ✅                                  |
